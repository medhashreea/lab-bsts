...
public class SimpleBST<K, V> implements SimpleMap<K, V> {
  ...
  public V set(K key, V value) {
    ...
    if (containsKey(key)) {
      BSTNode<K, V> newNode = set(key, value, null, root);
      // get KV pair
      this.cachedValue = newNode.value;
      newNode.value = value;
      // set KV pair value to new value
      return this.cachedValue;
    }

    set(key, value, null, root);
  ...
  BSTNode<K, V> set(K key, V value, BSTNode<K, V> prev, BSTNode<K, V> node) {
    if (node == null) {
      BSTNode<K, V> newNode = new BSTNode<K, V>(key, value);
      this.size++;
      if (prev != null) {
        int comp = comparator.compare(key, prev.key);
        if (comp < 0) {
          prev.left = newNode;
        } else {
          prev.right = newNode;
    ...
    if (comp == 0) {
      ...
      return set(key, value, node, node.left);
    } else {
      return set(key, value, node, node.right);
    }
  } // set(K, BSTNode<K,V>)
